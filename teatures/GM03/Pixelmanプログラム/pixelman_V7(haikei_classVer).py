...
class Pixelman :
    ...
    def update(self,anime) :
        ...
        self.anime.haikei.update(self.anime,self.bn_x,self.bn_vy)
...
class Haikei :
    ...
    def update(self,anime,x,y) :
        self.pos_x = x / self.pos_rate
        self.pos_start = x % 16

    def draw(self) :
       for i in range(0,256,16) :
            pyxel.blt(i                           , 0 ,1,
                      self.pos_ini+self.pos_start , 0,
                      self.pos_ini+self.pos_end   , 191 ,0 )
            pyxel.blt(i+(16-self.pos_start)       , 0 ,1,
                      self.pos_ini                , 0,
                      self.pos_start              , 191 ,0 )

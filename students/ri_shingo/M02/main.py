...
class Pixelman:
    ...
    def update(self, anime):
        Hanabi(self.anime, random.randint(0,Anime.SCREEN_WIDTH), random.randint(Anime.SCREEN_HIGHT-32,Anime.SCREEN_HIGHT))
        if self.bn_ptn ==0:
...
class Hanabi :
    def __init__(self,anime,x,y) :
        ...
        self.hana_x = x = x + 8
        self.hana_y = y
        self.hana_cut = random.randint(-20,100)
        self.hana_tim = 152

    def update(self):
        self.hana_cut += 1
        if self.hana_cut < self.hana_tim :
            self.hana_y -= 1
            self.hana_ptn = 0
        elif self.hana_cut < self.hana_tim+30 :
            self.hana_ptn = 1
        elif self.hana_cut < self.hana_tim+60 :
            self.hana_ptn = 2
        elif self.hana_cut < self.hana_tim+90 :
            self.hana_ptn = 3
        else :
